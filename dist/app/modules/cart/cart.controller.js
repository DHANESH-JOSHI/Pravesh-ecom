...
exports.addToCart = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const populatedCart = await cart_model_1.Cart.findOne({ user: userId }).populate({ path: 'items.product', select: 'name thumbnail', match: { isDeleted: false } });
...
exports.updateCartItem = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const updatedCart = await cart_model_1.Cart.findOne({ user: userId }).populate({ path: 'items.product', select: 'name thumbnail', match: { isDeleted: false } });
    await redis_1.redis.delete(redisKeys_1.RedisKeys.CART_BY_ID(String(cart._id)));
    await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.CART_BY_USER_ANY(String(userId)));
    await redis_1.redis.delete(redisKeys_1.RedisKeys.CART_SUMMARY_BY_USER(String(userId)));
    await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.USER_ANY(String(userId)));
...
exports.checkoutCart = (0, utils_1.asyncHandler)(async (req, res) => {
    const userId = req.user?._id;
    const cart = await cart_model_1.Cart.findOne({ user: userId }).populate({ path: 'items.product', select: 'isDeleted status stock name', match: { isDeleted: false } });
    ...
    await redis_1.redis.delete(redisKeys_1.RedisKeys.CART_SUMMARY_BY_USER(String(userId)));
    await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.USER_ANY(String(userId)));
    res.status(http_status_1.default.OK).json(new ApiResponse(http_status_1.default.OK, 'Checkout successful', {}));
    return;
});
