...
exports.createBrand = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const categoriesWithBrand = await category_model_1.Category.find({ brands: brand._id, isDeleted: false }).select('_id');
    for (const category of categoriesWithBrand) {
        ...
        if (!alreadyInvalidated) {
            await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.CATEGORY_ANY(categoryIdStr));
        }
    }
...
exports.updateBrand = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const categoriesWithBrand = await category_model_1.Category.find({ brands: id, isDeleted: false }).select('_id');
    for (const category of categoriesWithBrand) {
