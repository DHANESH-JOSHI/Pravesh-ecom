...
exports.getAllBlogs = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const cachedBlogs = await redis_1.redis.get(cacheKey);
    if (cachedBlogs) {
        return res.status(http_status_1.default.OK).json(new ApiResponse(http_status_1.default.OK, `Retrieved blogs`, cachedBlogs));
    }
    const filter = {};
    if (search) {
    ...
    const [posts, total] = await Promise.all([
        blog_model_1.Blog.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)),
    ...
    res.status(http_status_1.default.OK).json(new ApiResponse(http_status_1.default.OK, `Retrieved blogs`, result));
    return;
});
