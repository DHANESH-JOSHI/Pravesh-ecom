...
exports.createCategory = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const brandsWithCategory = await brand_model_1.Brand.find({ categories: category._id, isDeleted: false }).select('_id');
...
exports.updateCategory = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const brandsWithCategory = await brand_model_1.Brand.find({ categories: categoryId, isDeleted: false }).select('_id');
    for (const brand of brandsWithCategory) {
        await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.BRAND_ANY(String(brand._id)));
    }
    if (brandsWithCategory.length > 0) {
...
exports.deleteCategory = (0, utils_1.asyncHandler)(async (req, res) => {
    ...
    const brandsWithCategory = await brand_model_1.Brand.find({ categories: id, isDeleted: false }).select('_id');
    for (const brand of brandsWithCategory) {
        await redis_1.redis.deleteByPattern(redisKeys_2.RedisPatterns.BRAND_ANY(String(brand._id)));
    }
    if (brandsWithCategory.length > 0) {
